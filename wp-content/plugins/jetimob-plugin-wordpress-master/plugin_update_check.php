...
class PluginUpdateChecker_2_0 {
    ...
    public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
        ...
        if ( $isRelevant && current_user_can('update_plugins') ) {
            ...
            $linkText = apply_filters('puc_manual_check_link-' . $this->slug, 'Verificar atualização');
            if ( !empty($linkText) ) {
    ...
    public function displayManualCheckResult() {
        if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
            ...
            if ( $status == 'no_update' ) {
                $message = 'O plugin Jetimob está atualizado.';
            } else if ( $status == 'update_available' ) {
                $message = 'Uma nova versão do plugin Jetimob está disponível.';
            } else {
                $message = sprintf('Resposta desconhecida do servidor de atualização: "%s"', htmlentities($status));
            }
            printf(
...
class PluginInfo_2_0 {
    ...
    public static function fromJson($json, $triggerErrors = false){
        ...
        if ( empty($apiResponse) || !is_object($apiResponse) ){
            if ( $triggerErrors ) {
                trigger_error(
                    "Falha na busca dos metadados do plugin. Tente validar o arquivo .json em http://jsonlint.com/",
                    E_USER_NOTICE
                );
            }
            return null;
        ...
        if ( !$valid ){
            if ( $triggerErrors ) {
                trigger_error(
                    "Os metadados não contém as chaves obrigatórias de nome e/ou versão.",
                    E_USER_NOTICE
                );
            }
            return null;
